    ...
    public class CloseMatchController : ApiController
    {
        ...
        public HttpResponseMessage Post(JObject matchData)
        {
            ...
            try
            {
                ...
                var m1 = db.Match.Where(m => m.league_id == match.league_id && m.match_date >= now).OrderBy(a => a.match_date).Select(x => new { x.match_id, x.league_id, x.lng, x.lat, x.match_date, x.match_time, x.team_color1, x.team_color2 }).FirstOrDefault();
